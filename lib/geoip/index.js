...
var GeoIP = exports.GeoIP = function(filename) {
  if (!(this instanceof  GeoIP)) { return new GeoIP(filename) }

  this.fd = fs.openSync(filename, 'r')
  var size = fs.fstatSync(this.fd).size
  this.buffer = mmap.map(size, mmap.PROT_READ, mmap.MAP_SHARED, this.fd, 0)
  this._seek(0)
  this._setupSegments()
}
...
GeoIP.prototype._setupSegments = function() {
  ...
  for (var i = 0; i < consts.STRUCTURE_INFO_MAX_SIZE; ++i) {
    this._seek(this.buffer.length - 3 - i)
      ...
      switch (this._databaseType) {
        ...
        case consts.ASNUM_EDITION:
          ...
          for (var j = 0; j < consts.SEGMENT_RECORD_LENGTH; ++j) {
            this._databaseSegments += this._read() << (j * 8)
          }
